...
} from "@/apps/interfaces/AppInterface";
...
async function getPatientDashboardAlerts(
  patient: any
): Promise<GeneralDataInterface[]> {
  const sideEffects: Observation[] = await PatientAlerts.alertSideEffects(
    patient.getID()
  );
...
function orderToString(order: Order) {
  ...
  const status = OrderService.isHighViralLoadResult(result)
    ? '(<b style="color: #eb445a;">High</b>)'
    : "";
...
function confirmationSummary(patient: any, program: any) {
  return {
    ...
    GUARDIAN: async () => {
      const req = await RelationshipService.getGuardianDetails(patient.getID());
      if (req) {
