...
export default defineComponent({
  ...
  setup() {
    ...
    const buildDrillableCell = (patients: any[]) => ({
    ...
    const buildCells = (data: Record<string, any>) => {
      ...
      return {
        "underThree": buildDrillableCell(patients.filter(id => data[id]["prescribed_days"] < 90)),
        "betweenThreeAndFive": buildDrillableCell(patients.filter(id => data[id]["prescribed_days"] >= 90 && data[id]["prescribed_days"] <= 150)),
        "overSix": buildDrillableCell(patients.filter(id => data[id]["prescribed_days"] > 150)),
      }
    }
