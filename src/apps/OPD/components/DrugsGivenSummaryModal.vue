<template>
  <ion-header>
    <ion-toolbar>
      <ion-title>Durgs Given with prescription Report Summary</ion-title>
      <ion-button color="danger" slot="end" @click="closeModal()">X</ion-button>
    </ion-toolbar>
  </ion-header>
  <ion-content class="ion-padding">
    <ion-grid>
      <ion-row>
        <ion-col><h3>Count</h3></ion-col>
        <ion-col><h3>Drug Name</h3></ion-col>
        <ion-col><h3>Quantity</h3></ion-col>
      </ion-row>
      <ion-list>
        <ion-item v-for="(item, count) in list" :key="item">
          <ion-col>
            {{ count +1 }}
          </ion-col>
          <ion-col>
            {{ item.label }}
          </ion-col>
          <ion-col>
            {{ item.value}}
          </ion-col>
        </ion-item>
      </ion-list>
    </ion-grid>
  </ion-content>
</template>

<script lang="ts">
import { defineComponent, PropType } from 'vue'
import { modalController } from '@ionic/core'

export default defineComponent({
  props: {
    list: Object as PropType<any[]>
  },
  setup() {
    const closeModal = async () => await modalController.dismiss()
    
    return {
      closeModal
    }
  },
})
</script>
